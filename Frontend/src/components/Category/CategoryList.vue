      ...
      <v-data-table-server :headers="headers" :items="categories" :search="search || undefined"
          ...
          <tr>
            ...
            <td class="text-center">
              <v-tooltip v-bind="tooltipProps" text="Editar" location="bottom">
                <template v-slot:activator="{ props }">
                  <v-btn v-bind="props" v-if="canEdit && (item as Category).statusCategory == 'Activo'" icon
                    variant="text" color="indigo" size="small" @click="$emit('edit-category', item)">
              ...
              <v-tooltip v-bind="tooltipProps" text="Inactivar" location="bottom">
                <template v-slot:activator="{ props }">
                  <v-btn v-bind="props" v-if="canEdit && (item as Category).statusCategory == 'Activo'" icon
                    variant="text" color="red" size="small" @click="$emit('open-modal', { category: item, action: 2 })">
        ...
        <template v-slot:top>
          <v-toolbar>
            ...
            <v-tooltip v-bind="tooltipProps" text="Agregar Categoría" location="bottom">
              <template v-slot:activator="{ props }">
...
interface Props extends Omit<BaseListProps<Category>, 'items' | 'totalItems'> {
  categories: Category[];
