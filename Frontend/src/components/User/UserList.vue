      ...
      <v-data-table-server :headers="headers" :items="users" :search="search || undefined" :items-per-page-text="pages"
        ...
        <template v-slot:item="{ item }">
          <tr>
            ...
            <td>
              <v-chip :color="(item as User).statusUser === 'Activo' ? 'green' : 'red'"
                :variant="(item as User).statusUser === 'Activo' ? 'tonal' : 'tonal'" size="small" rounded="lg">
                <v-icon :icon="(item as User).statusUser === 'Activo' ? 'mdi:mdi-check-circle' : 'mdi:mdi-close-circle'"
                  start size="12"></v-icon>
                {{ (item as User).statusUser }}
              </v-chip>
            </td>
            <td class="text-center">
              <v-tooltip v-bind="tooltipProps" text="Editar" location="bottom">
                <template v-slot:activator="{ props }">
                  <v-btn v-bind="props" v-if="canEdit && (item as User).statusUser == 'Activo'" icon variant="text"
                    color="blue-darken-1" size="small" @click="$emit('edit-user', item)">
              ...
              <v-tooltip v-bind="tooltipProps" text="Activar" location="bottom">
                <template v-slot:activator="{ props }">
                  <v-btn v-bind="props" v-if="canEdit && (item as User).statusUser == 'Inactivo'" icon variant="text"
                    color="green-darken-1" size="small" @click="$emit('open-modal', { user: item, action: 1 })">
              ...
              <v-tooltip v-bind="tooltipProps" text="Inactivar" location="bottom">
                <template v-slot:activator="{ props }">
                  <v-btn v-bind="props" v-if="canEdit && (item as User).statusUser == 'Activo'" icon variant="text"
                    color="red-darken-1" size="small" @click="$emit('open-modal', { user: item, action: 2 })">
              ...
              <v-tooltip v-bind="tooltipProps" text="Eliminar" location="bottom">
                <template v-slot:activator="{ props }">
                  <v-btn v-bind="props" v-if="canDelete" icon variant="text" color="blue-grey-darken-1" size="small"
                    @click="$emit('open-modal', { user: item, action: 0 })">
        ...
        <template v-slot:top>
          <v-toolbar>
            <v-toolbar-title> <v-avatar color="indigo" size="36" class="mr-3">
              ...
              </v-avatar>Gestión de Usuarios</v-toolbar-title>
            <v-spacer></v-spacer>
            ...
            <v-tooltip v-bind="tooltipProps" text="Agregar Usuario" location="bottom">
              <template v-slot:activator="{ props }">
