        ...
        <template v-slot:item="{ item }">
          <tr>
            ...
            <td class="text-center">
              <v-btn v-if="canEdit && (item as User).statusUser == 'Activo'" icon variant="text" color="indigo"
                size="small" title="Editar" @click="$emit('edit-user', item)">
                <v-icon icon="mdi-pencil-outline" size="22"></v-icon>
              </v-btn>
              <v-btn v-if="canEdit && (item as User).statusUser == 'Inactivo'" icon variant="text" color="green"
                size="small" title="Activar" @click="$emit('open-modal', { user: item, action: 1 })">
                <v-icon icon="mdi-check-circle-outline" size="22"></v-icon>
              </v-btn>
              <v-btn v-if="canEdit && (item as User).statusUser == 'Activo'" icon variant="text" color="red"
                size="small" title="Inactivar" @click="$emit('open-modal', { user: item, action: 2 })">
