        ...
        <template v-slot:item="{ item }">
          <tr>
            ...
            <td class="text-center">
              <v-btn v-if="canEdit && (item as Store).statusStore == 'Activo'" icon="edit" variant="text"
                @click="$emit('edit-store', item)" size="small" title="Editar">
              </v-btn>
              <template v-if="canEdit && (item as Store).statusStore == 'Inactivo'">
                <v-btn icon="check" variant="text" @click="$emit('open-modal', { store: item, action: 1 })" size="small"
                  title="Activar">
                </v-btn>
              </template>
              <template v-if="canEdit && (item as Store).statusStore == 'Activo'">
                <v-btn icon="block" variant="text" @click="$emit('open-modal', { store: item, action: 2 })" size="small"
                  title="Inactivar">
                </v-btn>
              </template>
