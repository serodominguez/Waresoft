  ...
  <div>
    <v-card elevation="2">
      <v-data-table-server :headers="headers" :items="stores" :search="search || undefined" :items-per-page-text="pages"
        ...
        <template v-slot:item="{ item }">
          <tr>
            ...
            <td class="text-center">
              <v-btn v-if="canEdit && (item as Store).statusStore == 'Activo'" icon="edit" color="indigo" variant="text"
                @click="$emit('edit-store', item)" size="small" title="Editar">
              </v-btn>
              <template v-if="canEdit && (item as Store).statusStore == 'Inactivo'">
                <v-btn icon="check" color="green" variant="text" @click="$emit('open-modal', { store: item, action: 1 })" size="small"
                  title="Activar">
                </v-btn>
              </template>
              <template v-if="canEdit && (item as Store).statusStore == 'Activo'">
                <v-btn icon="block" color="red" variant="text" @click="$emit('open-modal', { store: item, action: 2 })" size="small"
                  title="Inactivar">
                </v-btn>
              </template>
        ...
        <template v-slot:top>
          <v-toolbar>
            <v-toolbar-title>Gestión de Establecimientos</v-toolbar-title>
            <v-spacer></v-spacer>
