        ...
        <v-form ref="formRef" v-model="valid">
          <v-container class="pa-0">
            <v-row density="compact">
              ...
              <v-col cols="6" md="6">
                <v-text-field color="indigo" variant="outlined" density="compact"v-model="localInventory.minimumStock" 
                 type="number" :rules="[rules.required]" label="Minimo" ref="minimumFieldRef"
              ...
              <v-col cols="6" md="6">
                <v-text-field color="indigo" variant="outlined" density="compact" v-model.number="localInventory.price"
                  counter="5" type="number" :rules="[rules.required]" :maxlength="5" label="Precio" ref="priceFieldRef"
                  @focus="($event.target as HTMLInputElement).select()" required />
              </v-col>
            </v-row>
          </v-container>
...
const minimumFieldRef = ref<HTMLInputElement>();
...
watch(
  ...
  (newValue) => {
    if (newValue) {
      nextTick(() => {
        if (minimumFieldRef.value) {
          minimumFieldRef.value.focus();
        }
      });
    }
  }
...
const savePrice = async () => {
  ...
  try {
    ...
    handleApiError(error, 'Error al actualizar');
  } finally {
    saving.value = false;
  }
};
