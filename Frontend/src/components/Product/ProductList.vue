        ...
        <template v-slot:item="{ item }">
          <tr>
            ...
            <td class="text-center">
              <v-btn v-if="canEdit && (item as Product).statusProduct == 'Activo'" icon="edit" color="indigo" variant="text"
                @click="$emit('edit-product', item)" size="small" title="Editar">
              </v-btn>
              <template v-if="canEdit && (item as Product).statusProduct == 'Inactivo'">
                <v-btn icon="check" color="green" variant="text" @click="$emit('open-modal', { product: item, action: 1 })"
                  size="small" title="Activar">
                </v-btn>
              </template>
              <template v-if="canEdit && (item as Product).statusProduct == 'Activo'">
                <v-btn icon="block" color="red" variant="text" @click="$emit('open-modal', { product: item, action: 2 })"
