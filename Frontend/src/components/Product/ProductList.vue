      ...
      <v-data-table-server :headers="headers" :items="products" :search="search || undefined"
        ...
        <template v-slot:item="{ item }">
          <tr>
            ...
            <td>
              <v-chip :color="(item as Product).statusProduct === 'Activo' ? 'green' : 'red'"
                :variant="(item as Product).statusProduct === 'Activo' ? 'tonal' : 'tonal'" size="small" rounded="lg">
                <v-icon
                  :icon="(item as Product).statusProduct === 'Activo' ? 'mdi:mdi-check-circle' : 'mdi:mdi-close-circle'"
                  start size="12"></v-icon>
                {{ (item as Product).statusProduct }}
              </v-chip>
            </td>
            <td class="text-center">
              <v-btn v-if="canEdit && (item as Product).statusProduct == 'Activo'" icon variant="text" color="indigo"
                size="small" title="Editar" @click="$emit('edit-product', item)">
                <v-icon icon="mdi-pencil-outline" size="22"></v-icon>
              </v-btn>
              <v-btn v-if="canEdit && (item as Product).statusProduct == 'Inactivo'" icon variant="text" color="green"
                size="small" title="Activar" @click="$emit('open-modal', { product: item, action: 1 })">
                <v-icon icon="mdi-check-circle-outline" size="22"></v-icon>
              </v-btn>
              <v-btn v-if="canEdit && (item as Product).statusProduct == 'Activo'" icon variant="text" color="red"
                size="small" title="Inactivar" @click="$emit('open-modal', { product: item, action: 2 })">
        ...
        <template v-slot:top>
          <v-toolbar>
            <v-toolbar-title> <v-avatar color="purple-darken-1" size="36" class="mr-3">
                <v-icon icon="mdi-package" color="white" size="18"></v-icon>
              </v-avatar>Gestión de Productos</v-toolbar-title>
            <v-spacer></v-spacer>
