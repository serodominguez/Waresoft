<template>
  <v-card v-if="isOpen" elevation="2">
    <v-toolbar>
      ...
      <v-spacer></v-spacer>
      <v-chip v-if="localReceipt.idReceipt" :color="localReceipt.statusReceipt === 'Completado' ? 'green' : 'red'"
        text-color="white" size="small" label class="mr-2">
        Estado: {{ localReceipt.statusReceipt }}
      </v-chip>
    </v-toolbar>
    <v-card-text>
      <v-form ref="formRef" v-model="valid">
        <v-row align="center">
          ...
          <v-col v-if="localReceipt.type === 'Adquisición'" cols="12" md="2">
            ...
            <v-text-field v-else color="indigo" variant="outlined" density="compact" v-model="localReceipt.documentType"
          ...
          <v-col v-if="!localReceipt.idReceipt && localReceipt.type === 'Adquisición'" cols="12" md="2">
            <v-date-input color="indigo" locale="es" placeholder="dd/mm/yyyy" v-model="localReceipt.documentDate"
          ...
          <v-col v-if="localReceipt.type === 'Adquisición'" cols="12" md="2">
            ...
            <v-text-field v-else color="indigo" variant="outlined" density="compact" v-model="localReceipt.companyName"
          ...
          <v-col v-if="!localReceipt.idReceipt" cols="12" md="2" style="padding-bottom: 22px;">
            <v-tooltip v-bind="tooltipProps" text="Seleccionar Producto" location="bottom">
              <template v-slot:activator="{ props }">
                <v-btn v-bind="props" fab dark color="indigo" elevation="4" @click="openProductModal">
        ...
        <v-col v-if="!localReceipt.idReceipt" cols="12" class="d-flex justify-end mt-4 pr-4">
...
const close = () => {
  ...
  emit('close');
    ...
    router.replace({ name: 'goodsreceipt' });
  }
};
