<template>
  <v-data-table :headers="headers" :items="permissions" :loading="loading" loading-text="Cargando permisos..."
    ...
    <template v-slot:header.permissions.eliminar="{ column }">
      <div class="d-flex align-center gap-1">
        <v-checkbox v-model="allChecked.eliminar" color="indigo" hide-details
          :indeterminate="isIndeterminate('eliminar')" @change="toggleAll('eliminar')" density="compact"
    ...
    <template v-slot:header.permissions.descargar="{ column }">
      <div class="d-flex align-center gap-1">
        <v-checkbox v-model="allChecked.descargar" color="indigo" hide-details
          :indeterminate="isIndeterminate('descargar')" @change="toggleAll('descargar')" density="compact"
          :disabled="!hasPermissions" />
        {{ column.title }}
      </div>
    </template>
