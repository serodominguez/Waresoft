<template>
  <div>
      ...
      <v-data-table-server :headers="headers" :items="transfers" :search="search || undefined"
        ...
        <template v-slot:top>
          <v-toolbar>
            ...
            <v-tooltip v-bind="tooltipProps" text="Registrar Traspaso" location="bottom">
              <template v-slot:activator="{ props }">
    ...
    <CommonFilters v-model="drawerModel" :filters="filterOptions" v-model:selected-filter="selectedFilterModel"
      :status-options="TransferStatusOptions" v-model:state="stateModel" v-model:start-date="startDateModel"
