      ...
      <v-data-table-server :headers="headers" :items="suppliers" :search="search || undefined"
            ...
            <td class="text-center">
              <v-tooltip v-bind="tooltipProps" text="Editar" location="bottom">
                <template v-slot:activator="{ props }">
                  <v-btn v-bind="props" v-if="canEdit && (item as Supplier).statusSupplier == 'Activo'" icon
                    variant="text" color="indigo" size="small" @click="$emit('edit-supplier', item)">
              ...
              <v-tooltip v-bind="tooltipProps" text="Activar" location="bottom">
                <template v-slot:activator="{ props }">
                  <v-btn v-bind="props" v-if="canEdit && (item as Supplier).statusSupplier == 'Inactivo'" icon
                    variant="text" color="green" size="small"
                    @click="$emit('open-modal', { supplier: item, action: 1 })">
              ...
              <v-tooltip v-bind="tooltipProps" text="Inactivar" location="bottom">
                <template v-slot:activator="{ props }">
                  <v-btn v-bind="props" v-if="canEdit && (item as Supplier).statusSupplier == 'Activo'" icon
                    variant="text" color="red" size="small" @click="$emit('open-modal', { supplier: item, action: 2 })">
        ...
        <template v-slot:top>
          <v-toolbar>
            ...
            <v-tooltip v-bind="tooltipProps" text="Agregar Proveedor" location="bottom">
              <template v-slot:activator="{ props }">
