        ...
        <template v-slot:item="{ item }">
          <tr>
            ...
            <td class="text-center">
              <v-btn v-if="canEdit && (item as Supplier).statusSupplier == 'Activo'" icon="edit" color="indigo"
              ...
              <template v-if="canEdit && (item as Supplier).statusSupplier == 'Activo'">
                <v-btn icon="block" color="red" variant="text"
                  @click="$emit('open-modal', { supplier: item, action: 2 })" size="small" title="Inactivar">
                </v-btn>
              </template>
