        ...
        <template v-slot:item="{ item }">
          <tr>
            ...
            <td class="text-center">
              <v-btn v-if="canEdit && (item as Customer).statusCustomer == 'Activo'" icon="edit" color="indigo"
                variant="text" @click="$emit('edit-customer', item)" size="small" title="Editar">
              </v-btn>
              <template v-if="canEdit && (item as Customer).statusCustomer == 'Inactivo'">
                <v-btn icon="check" color="green" variant="text"
                  @click="$emit('open-modal', { customer: item, action: 1 })" size="small" title="Activar">
                </v-btn>
              </template>
              <template v-if="canEdit && (item as Customer).statusCustomer == 'Activo'">
                <v-btn icon="block" color="red" variant="text"
                  @click="$emit('open-modal', { customer: item, action: 2 })" size="small" title="Inactivar">
                </v-btn>
              </template>
