        ...
        <template v-slot:item="{ item }">
          <tr>
            ...
            <td class="text-center">
              <v-btn v-if="canEdit && (item as Customer).statusCustomer == 'Activo'" icon variant="text" color="indigo"
                size="small" title="Editar" @click="$emit('edit-customer', item)">
                <v-icon icon="mdi-pencil-outline" size="22"></v-icon>
              </v-btn>
              <v-btn v-if="canEdit && (item as Customer).statusCustomer == 'Inactivo'" icon variant="text" color="green"
                size="small" title="Activar" @click="$emit('open-modal', { customer: item, action: 1 })">
                <v-icon icon="mdi-check-circle-outline" size="22"></v-icon>
              </v-btn>
              <v-btn v-if="canEdit && (item as Customer).statusCustomer == 'Activo'" icon variant="text" color="red"
                size="small" title="Inactivar" @click="$emit('open-modal', { customer: item, action: 2 })">
